<script>
	...
	let errors;
	let profile;
	let name, address, contact, username, email, avatar_url, avatar_file;
				...
				if (response.status) {
					getProfile(() => {
						return setFlash({
							...
							type: 'success',
							redirect: '/corporate/settings/profile'
						});
					});
				}
			})
	...
	const handleSubmit = () => {
		...
		ApiController.sendRequest({
			...
			data: { name, address, contact, username, email },
			authToken: user.token
		})
			.then((response) => {
				if (response.error) {
	...
	const getProfile = (callback = null) => {
		ApiController.sendRequest({
			...
			if (response.status) {
				...
				kuki.name = response.data.info.name;
				kuki.address = response.data.info.address;
				kuki.contact = response.data.info.contact;
...
<div class="flex">
	<Sidebar active="else" role="Corporate Admin" bind:isSidebarOpen />
	<div class="neutral-wrapper px-3">
		<Navbar active="" variant="inside" pageTitle="Pengaturan Akun" bind:user bind:isSidebarOpen />
				...
				<div class="card p-standard radius-sm">
					<div class="card-body gap-4">
						<Tab
							type="link"
							menus={[
								{ title: 'Profil', href: '/corporate/settings/profile', active: true },
								{ title: 'Kata Sandi', href: '/corporate/settings/password', active: false }
							]}
						/>
						...
						<div class="row">
							<div class="col-12 col-md-8">
								<div class="flex-column gap-4">
									{#if profile}
										<div class="flex-column gap-3">
											...
											<InputField
												...
												rules={[{ required: true }]}
												error={errors ? (errors.contact ? errors.contact : '') : ''}
											/>
									...
									{/if}
									{#if profile && (name != profile.name || address != profile.address || contact != profile.contact || username != profile.username)}
