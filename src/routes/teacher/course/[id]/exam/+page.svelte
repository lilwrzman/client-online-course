<script>
	...
	import { Circle, List, PlusLg, RecordCircle, Trash3, X } from "svelte-bootstrap-icons";
    ...
    const handleSubmit = () => {
        ...
        let datas = {
            type: "Exam", title: title, description: description, 
        ...
        ApiController.sendRequest({
            ...
            if(response.error){
                ...
                showSpinner = false
                toastData = { title: "Gagal", message: "Mohon periksa kembali data Ujian!", color: 'toast-danger' }
                toastVisible = true
                return
            }
            ...
            <div class="container flex-column py-4 gap-5" style="flex-grow: 1;">
                ...
                <div class="row justify-content-center">
                    <div class="flex gap-2 col-12 col-md-10 p-0">
                        ...
                        <a href="/teacher/course/{id}?active=Submateri" class="body-medium-semi-bold tc-neutral-disabled">{detail.title}</a>
                        <div class="body-medium-semi-bold tc-neutral-disabled">/</div>
                        <a href="/teacher/course/{id}/exam#" class="body-medium-semi-bold tc-primary-main">Ujian</a>
                    </div>
                </div>
                    ...
                    <div class="card radius-sm p-5 card-cap-primary col-12 col-md-10">
                        <div class="card-body gap-3">
                            <InputField bind:value={title} rules={[{required: true}]} error={errors ? errors.title ? errors.title : '' : ''}
                                label="Judul Ujian" labelClass="body-medium-semi-bold" id="title" placeholder="Masukkan judul"/>
                            <InputField bind:value={passing_score} rules={[{required: true}]} error={errors ? errors.passing_score ? errors.passing_score : '' : ''}
                                type="number" label="Nilai KKM Ujian" labelClass="body-medium-semi-bold" id="passing_score" placeholder="Masukkan nilai kriteria ketuntasan minimal"/>
                            <InputField bind:value={description} rules={[{required: true}]} error={errors ? errors.description ? errors.description : '' : ''}
                                type="tinymce" label="Deskripsi Ujian" labelClass="body-medium-semi-bold" id="description" placeholder="Masukkan deskripsi"/>
                        </div>
                    </div>
                </div>

                <div class="row gap-4 position-relative justify-content-center" use:dragHandleZone={{ items, flipDurationMs }} on:consider={handleDndConsider} on:finalize="{handleSort}">
                ...
                <div class="row gap-4 position-relative justify-content-center">
                    ...
                    <div class="flex justify-content-end gap-2 col-12 col-md-10 p-0">
                        ...
                        <Button classList="btn btn-main" onClick={handleSubmit}>Simpan Ujian</Button> 
                    </div>
                </div>
                {/if}
            </div>
